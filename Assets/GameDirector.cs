...
{
    //효과음 다중 재생을위하여 오디오클립을 추가함
    public AudioClip BonusSound;            //하트 먹는 사운드클립
    public AudioClip FireHitSound;          //불 맞는 사운드클립
    public AudioClip WaterHitSound;         //물 맞는 사운드클립
    ...
    public void f_HpBarFireDecrease()
    {
        ...
        f_FireHitSound();   //불에 맞는 사운드 출력
        f_NextScene();      //체력이 0 이면 다음씬 불러오는 메소드
    ...
    {
        audioSource.volume = 0.8f; //효과음 볼륨 0.8로 지정
        audioSource.PlayOneShot(BonusSound); //PlayOneShot 메소드를 통해 효과음 재생중 다른 물체에 피격되도 효과음의 끝까지 재생하도록 사용
    }
